...
export class HealthService {
  constructor(
    ...
    private readonly redisService: RedisService<RedisClientT>,
    private readonly redisHealth: RedisHealthIndicator,
  ) {}

  checkHealth(): Promise<HealthCheckResult> {
    const redisMemoryThreshold = this.configService.get<number>('REDIS_MEMORY_THRESHOLD_BYTES', 1000000000, { infer: true }) as number; // Default to 1GB
    const diskThreshold = this.configService.get<number>('DISK_THRESHOLD_PERCENT', 0.75, { infer: true }) as number; // Default to 75%
